...
class Trial(object):
    def __init__(self):
        ...
        self.blocks_per_session = 2                         # Number of blocks per session stored in parameters.txt
        ...
        self.icon_color = 0
        self.trial_type = [1, 2, 3, 4, 1, 4, 2, 3, 2, 3,
                           2, 1, 1, 3, 2, 4, 1, 4, 3, 4]    # Trial Type = 1: Blue Icon Right, 2: Orange Icon Left, 3: Blue Icon Left, 4: Orange Icon Right
    ...
    def trial_duration(self):
        ...
        if seconds > duration and SELECT != -1:
            ...
            #sound(False)
            self.write(data_file, "null", self.response_time(), 2)
            start_time = pygame.time.get_ticks()
            #self.trial_number -= 1
...
#----------------------------------------------------------------
    ...
    def run_trial(self):
        ...
        cursor.draw(screen)
        if self.session_type == 2:                                      # If its the Half Task
            if self.icon_condition == 1:                                # If its a purple monkey
                
                    if cursor.position[0] <= 320:                       
                        moveCursor(cursor, only = 'right')              # Only let it go Right
                    else:
                        moveCursor(cursor, only = 'left, right')
                        
            elif self.icon_condition == 2:                              # Else if its orange monkey
                    if cursor.position[0] >= 480:
                        moveCursor(cursor, only = 'left')               # Only let it go Left
                    else:
                        moveCursor(cursor, only = 'left, right')
        else:
            moveCursor(cursor, only = 'left, right')
    ...
    def write(self, file, side, time_taken, correct):
        ...
        data = [monkey, today, now, icon_condition, session_type, self.block, self.trial_number, self.trial_type[self.trial_within_block], side, time_taken,
...
pygame.display.set_caption("Simon_Says_Testing")
...
writeLn(data_file, ['monkey', 'date', 'time', 'icon_condition', 'session_type', 'block', 'trial_number', 'trial_type',
