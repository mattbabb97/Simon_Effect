...
class Trial(object):
    ...
    def trial_duration(self):
        ...
        if seconds < duration:
            seconds = ((pygame.time.get_ticks() / 1000) - self.seconds - 2)
            print(seconds)
    ...
    def delay_duration(self):
        ...
        if seconds > 2 and SELECT1 != -1:
            seconds = seconds
        elif seconds > 2 and SELECT1 == -1:
            seconds = 0
            self.phase1 = False
            self.phase2 = True
    ...
    def response_time(self):
        seconds = 0
        if seconds < duration:
            seconds = ((pygame.time.get_ticks() / 1000) - self.seconds - 2.000)
